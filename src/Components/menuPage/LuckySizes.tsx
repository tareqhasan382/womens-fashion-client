...
const LuckySizes: React.FC = () => {
  const dispatch = useAppDispatch();
  const [hoveredProductId, setHoveredProductId] = useState<string | null>(null);
  const [limit] = useState<number>(20);
  const [page] = useState<number>(1);
  const [filterField] = useState<string>("LuckySizes");
    ...
    <>
      {isLoading ? (
        ...
        <div className=" w-full h-full flex flex-col items-center justify-center mx-auto mb-20 ">
          <h1 className=" uppercase text-xl font-bold my-5 ">Lucky Sizes</h1>
